...
class contentMod extends commonMod
{
    ...
    public function common_list_where()
    {
        ...
        //exit();
        if(!empty($copyfrom)){
            $where=' AND A.copyfrom="'.$copyfrom.'"';
            $where_url='-position-'.$copyfrom;
        }
        return array(
            'order'=>$order,
            'where'=>$where,
    ...
    public function common_list()
    {
        ...
        $admin_list=model('user')->admin_list();
        //print_r($admin_list);
        //exit();
    ...
    public function common_info($id,$status=false)
    {
        if($status){
            ...
            $action_name='添加';
            $action='add';
            $info['copyfrom']=model('user')->info($_SESSION[$this->config['SPOT'].'_user']);
            $info['copyfrom']=$info['copyfrom']['nicename'];
        }
    ...
    public function index()
    {
        ...
        //用户名对比
        $list['user2']=model('user')->current_user();
    ...
    {
        /*hook*/
        $user['copyfrom']=model('user')->info($_SESSION[$this->config['SPOT'].'_user']);
        $user['copyfrom']=$user['copyfrom']['nicename'];
